                        ...
                        @forelse ($unsolved_cases as $unsolved_case)
                            <a href="{{ route('cases.unsolved.show', $unsolved_case) }}" class="block group">
                                <div class="border border-primary rounded-lg p-4 shadow-sm bg-surface group-hover:shadow-lg group-hover:border-accent transition">
                                    ...
                                    <h2 class="text-xl font-semibold text-text group-hover:text-accent transition">{{ $unsolved_case->name }}</h2>
                                    <p class="text-sm text-text/60">Country: {{ $unsolved_case->country }}</p>
                                </div>
                            </a>
                        @empty
                            <p class="text-text/60">No unsolved case data available.</p>
                        @endforelse
                    </div>
                </div>
            </div>
